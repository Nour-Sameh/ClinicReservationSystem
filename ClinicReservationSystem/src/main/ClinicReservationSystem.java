...
// Handles user registration for practitioners or patients
  ...
  // Views all appointments of the current patient
    ...
    private static void editClinic() {
        ...
        System.out.println("5. Slot Duration");
        System.out.print("> ");
        int choice = in.nextInt();
        in.nextLine();
        ...
        switch (choice) {
            case 1 -> {
                ...
                clinic.setName(in.nextLine().trim());
            }
            case 2 -> {
                System.out.print("Enter new address: ");
                clinic.setAddress(in.nextLine().trim());
            }
            case 3 -> {
                System.out.print("Enter new price: ");
                clinic.setPrice(Double.parseDouble(in.nextLine().trim()));
            }
            case 4 -> {
                System.out.print("Enter new specialty (1-4): ");
                clinic.setDepartmentID(Integer.parseInt(in.nextLine().trim()));
            }
            case 5 -> {
                System.out.print("Enter new slot duration: ");
                clinic.getSchedule().setSlotDurationInMinutes(Integer.parseInt(in.nextLine().trim()));
                clinic.getSchedule().generateTimeSlots(); // إعادة توليد الفترات الزمنية
            }
    ...
    private static void cancelAppointment() {
        ...
        for (Appointment a : appointments) {
            if (a.getPatient() == cur_Patient) myApps.add(a);
        }
        ...
        for (int i = 0; i < myApps.size(); i++) {
            Appointment a = myApps.get(i);
            System.out.println((i + 1) + ". Clinic: " + a.getClinic().getName() +
                    " | Day/Time: " + a.getAppointmentDateTime());
        }
        ...
        if (idx <= 0 || idx > myApps.size()) return;

        Appointment toCancel = myApps.get(idx - 1);
        toCancel.getAppointmentDateTime().markAsAvailable();
        ...
        if (!clinic.getWaitingList().isEmpty()) {
            ...
            TimeSlot slot = toCancel.getAppointmentDateTime();
            slot.markAsBooked();
            Appointment newApp = new Appointment(nextPatient, clinic, slot);
            appointments.add(newApp);
            System.out.println("Waiting list patient " + nextPatient.getName() +
                    " booked automatically for this slot.");
        }
    }
}
